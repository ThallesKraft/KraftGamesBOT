...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if(!message.member.hasPermission("MANAGE_GUILD")) return message.channel.send(`❌ | ${ferinha_author} ${ferinha_msg_error_perm}`)

        let ferinha_canal = message.mentions.channels.first() || message.guild.channels.cache.get(args[0]);

        if(!ferinha_canal) return message.channel.send(`❌ | ${ferinha_author} ${ferinha_msg_error_ferinha_canal}`);
